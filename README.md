...
* альтернатива для CI: прокси с российским IP —
  `CADASTRE_PROXY=http://user:pass@host:port` (локально/Docker) или
  одноимённый repo secret для GitHub Actions.
* недоступность ПКК **не валит сборку**: карта соберётся из base+contours,
  в логе будет предупреждение.
...
* **Высоты**: подписи горизонталей — в **метрах** (оригинальный стиль
  OpenTopoMap конвертирует в футы, здесь конверсия убрана: карта
  метрическая).
...
* **Надёжность SRTM**: скачивание рельефа идёт отдельной фазой с
  таймаутами и ретраями (viewfinderpanoramas.org периодически подвешивает
  соединения), генерация — офлайн из кеша `hgt/` в один процесс
  (в pyhgtmap 4.1 параллельный режим дедлочится; `PYHGT_JOBS` — на свой
  страх и риск).
