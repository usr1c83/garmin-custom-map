...
`curl -C - --retry-all-errors`. Потоки/попытки одной загрузки:
`DOWNLOAD_CONNECTIONS` (4) и `DOWNLOAD_TRIES` (10, докачки aria2). Поверх
этого — внешние попытки на регион: `OSM_DOWNLOAD_ATTEMPTS` (10 в
`fetch_osm.sh`, каждая заново дёргает `download.sh` при браке файла) и
`HGT_DOWNLOAD_ATTEMPTS` (10 в `make_contours.sh`, фаза `--download-only`,
кеш `hgt/` копит прогресс между попытками). Каждый скачанный .pbf
дополнительно валидируется `osmium fileinfo -F pbf` (важно: без `-F pbf`
osmium определяет формат по расширению и на временных `*.tmp` всегда падает).

В плановом релизе (`release-scheduled.yml`) регионы собираются **по очереди**
(`strategy.max-parallel: 1`), а не все 24 сразу — чтобы не долбить Geofabrik
параллельными скачиваниями (риск бана/троттлинга по IP). Ускорить можно,
подняв `max-parallel`; упавший регион при `fail-fast: false` не блокирует
следующие, а в релиз публикуется всё, что успело собраться.
...
* **DEM и 180-й меридиан (Дальневосточный ФО)**: если тайл сплиттера
  содержит данные по обе стороны 180° (Чукотка), его bbox растягивается
  почти на весь мир, и DEM-секция превышает форматный лимит Garmin 256 МиБ:
  mkgmap падает с `SEVERE: The DEM section of the map or tile is too big`
  (1 MapFailedException; проявляется в зависимости от нарезки тайлов, т.е.
  от суточного экстракта — «вчера собиралось, сегодня нет»). Решение:
  `--dem-poly=region.poly` в `compile_layer.sh` — DEM обрезается границей
  региона, пустота вне полигона кодируется почти бесплатно. Запасной
  вариант — `with_dem: false` для региона в `release_regions.yaml`
  (или `-f with_dem=0` в build-map). Диагностика таких падений: артефакт
  `<регион>-logs` (выгружается при любом падении сборки) + консоль
  печатает контекст исключения из `mkgmap.log`.
* **osmium merge тысяч файлов убивает раннер**: merge держит буферы всех
  входов одновременно; на 2840 тайлах горизонталей ДВ ФО память 16-ГБ
  раннера кончается и VM снимает агент — job гибнет с «The runner has
  received a shutdown signal» ровно на строке `[contours] merging …`
  (три воспроизведения). Решение в `compile_layer.sh`: слияние партиями
  по `MERGE_BATCH` (300) файлов + финальный merge частей.
