...
`MAX_TRIES` (3) раз. Не помогло — последний рубеж: пересборка без
встроенного DEM (`WITH_DEM=0`, самая тяжёлая часть base-слоя). Если и
это не влезло, job падает с подсказкой разбить регион в
`release_regions.yaml`. Размер каждой карты выводится в Summary запуска.
...
увеличивает файл (тестовый район: 0.9 → 2.8 МБ); при превышении лимита
релиза size-guard сбрасывает DEM автоматически.
